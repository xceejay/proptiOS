...
// Update validation schema based on the audit fields
...
const SidebarAddAudit = props => {
  const { setAuditData, auditData, open, toggle } = props
    ...
    audit.addAudit(
      ...
      responseData => {
        ...
        const updatedRequestData = requestData.map(audit => {
          const matchingAudit = data.find(response => response.email === audit.email)
    ...
    >
      ...
      <Box sx={{ p: theme => theme.spacing(0, 6, 6) }}>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <FormControl fullWidth sx={{ mb: 4 }}>
            <InputLabel id='role-select'>Audit Type</InputLabel>
            <Select
              fullWidth
              value={role}
              id='select-role'
...
export default SidebarAddAudit
