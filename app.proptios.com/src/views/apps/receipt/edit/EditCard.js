...
const ReceiptAction = styled(Box)(({ theme }) => ({
  display: 'flex',
...
const EditCard = ({ data }) => {
  ...
  const [dueDate, setDueDate] = useState(data ? new Date(data.receipt.dueDate) : new Date())
  const [issueDate, setIssueDate] = useState(data ? new Date(data.receipt.issuedDate) : new Date())

  // ** Hook
  const theme = useTheme()
  useEffect(() => {
    axios.get(process.env.NEXT_PUBLIC_API_BASE_URL + '/apps/receipt/clients').then(response => {
    ...
    return (
      <Card>
        ...
        <CardContent sx={{ p: [`${theme.spacing(6)} !important`, `${theme.spacing(10)} !important`] }}>
          <Grid container>
            <Grid size={12} sm={6} sx={{ mb: { lg: 0, xs: 4 } }}>
              ...
              <Select size='small' value={selected} onChange={handleReceiptChange} sx={{ mb: 4, width: '200px' }}>
                {clients !== undefined &&
                  clients.map(client => (
        ...
        <CardContent sx={{ px: [6, 10] }}>
          <InputLabel htmlFor='receipt-note' sx={{ mb: 2, fontWeight: 500, fontSize: '0.875rem' }}>
            Note:
          </InputLabel>
          <TextField
            rows={2}
