...
const EditCard = ({ data }) => {
  ...
  useEffect(() => {
    axios.get(process.env.NEXT_PUBLIC_API_BASE_URL + '/apps/invoice/clients').then(response => {
    ...
    return (
      <Card>
        ...
        <CardContent sx={{ px: [6, 10] }}>
          ...
          <TextField rows={2} fullWidth multiline id='lease-note' defaultValue='Please sign the document. Thank You!' />
        </CardContent>
      </Card>
    )
  } else {
