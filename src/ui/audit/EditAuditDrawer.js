...
const EditAuditDrawer = props => {
  const { auditData, setAuditData, auditData, open, toggle, setLoading } = props
  const audit = useAudit()
  const properties = useProperties()
  const router = useRouter()
  const { id } = router.query

  // Updated validation schema to include audit-specific fields
  const schema = yup.object().shape({
    name: yup.string().min(3).required('Audit Name field is required'),
  ...
  const defaultValues = {
    ...
    tel_number: auditData?.tel_number,
    property_id: auditData?.property_id
  }
  ...
  //         console.log('FROM Edit audit PAGE: refreshing property Data', responseData)

  //         if (responseData?.status === 'FAILED') {
  //           alert(responseData.message || 'Failed to fetch properties')
  //         }

  //         setAuditData([...propertyData?.audit])
  //         setLoading(false)
  //       },
  //       error => {
  //         console.log(id)
  ...
  const onSubmit = formData => {
    setLoading(true)
    formData.unit_id = auditData.unit.id
    formData.property_id = auditData.property.id
    formData.id = auditData.id
    ...
    audit.editAudit(
      ...
      responseData => {
        ...
        const updatedRequestData = requestData.map(audit => {
          const matchingAudit = data.find(response => response.uuid === audit.uuid)
      ...
      error => {
        toast.error('Failed to edit audit', { duration: 3000 })
  ...
  const statusLabel = auditData?.status === 'active' ? 'Active' : 'Inactive'
  const statusColor = auditData?.status === 'active' ? 'success' : 'secondary'
          ...
          {/* <FormControl fullWidth sx={{ mb: 4 }}>
            <Controller
              ...
              render={({ field: { onChange, onBlur, value, ref } }) => (
                <Autocomplete
                  options={auditData.property_id}
                  getOptionLabel={unit => unit.name}
                  getOptionDisabled={unit => !!unit.audit_id}
                  onChange={(event, newValue) => {
                    // Pass the new value's id or an empty string to handle the form state
                    onChange(newValue ? newValue.id : '')
                  }}
                  value={auditData.property_id || null} // Set the selected value
...
export default EditAuditDrawer
