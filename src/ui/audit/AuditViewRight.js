...
const AuditViewRight = ({ tab, auditData }) => {
  ...
  const handleChange = (event, value) => {
    ...
    router
      .push({
        pathname: `/audit/manage/${id}/${value.toLowerCase()}`
      })
    ...
    <TabContext value={activeTab}>
      ...
      >
        <Tab
          ...
          value='transactions'
          label='Transactions'
          icon={<Icon fontSize='14px' icon='tabler:audit-check' />}
        />
        <Tab
          sx={{ fontSize: '13px' }}
          value='security'
