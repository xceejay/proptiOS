...
import { schema, Header } from './AddCommunicationDrawer'
import toast from 'react-hot-toast'

export const SidebarAddCommunication = props => {
  ...
  const onSubmit = data => {
    ...
    communication.addCommunication(
      ...
      responseData => {
        ...
        console.log('FROM Communication drawer PAGE:', data)

        if (data?.status === 'NO_RES') {
          console.log('NO results')
        } else if (data?.status === 'FAILED') {
          alert(data.message || 'Failed to add communication')
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h6'>Add Communication</Typography>
        <IconButton
          size='small'
          onClick={handleClose}
      ...
      <Box sx={{ p: theme => theme.spacing(0, 6, 6) }}>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <FormControl fullWidth sx={{ mb: 4 }}>
            <InputLabel id='role-select'>Communication Type</InputLabel>
            <Select
              fullWidth
              value={role}
              id='select-role'
