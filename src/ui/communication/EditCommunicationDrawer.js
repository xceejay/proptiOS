...
const EditCommunicationDrawer = props => {
  ...
  // Updated validation schema to include communication-specific fields
  const schema = yup.object().shape({
    name: yup.string().min(3).required('Communication Name field is required'),
  ...
  const defaultValues = {
    ...
    tel_number: communicationData?.tel_number,
    property_id: communicationData?.property_id
  }
  ...
  //         console.log('FROM Edit communication PAGE: refreshing property Data', responseData)

  //         if (responseData?.status === 'FAILED') {
  //           alert(responseData.message || 'Failed to fetch properties')
  //         }

  //         setCommunicationData([...propertyData?.communication])
  //         setLoading(false)
  //       },
  //       error => {
  //         console.log(id)
  ...
  const onSubmit = formData => {
    ...
    formData.id = communicationData.id
    ...
    communication.editCommunication(
      ...
      responseData => {
        ...
        const updatedRequestData = requestData.map(communication => {
          const matchingCommunication = data.find(response => response.uuid === communication.uuid)
      ...
      error => {
        toast.error('Failed to edit communication', { duration: 3000 })
  ...
  const statusLabel = communicationData?.status === 'active' ? 'Active' : 'Inactive'
  const statusColor = communicationData?.status === 'active' ? 'success' : 'secondary'
          ...
          <FormControl fullWidth sx={{ mb: 4 }}>
            <Controller
              ...
              render={({ field: { value, onChange, onBlur } }) => (
                <TextField
                  select
                  id='custom-select-communication-type'
                  value={value}
                  onChange={onChange}
                  onBlur={onBlur}
                  fullWidth
          ...
          {/* <FormControl fullWidth sx={{ mb: 4 }}>
            <Controller
              ...
              render={({ field: { onChange, onBlur, value, ref } }) => (
                <Autocomplete
                  options={communicationData.property_id}
                  getOptionLabel={unit => unit.name}
                  getOptionDisabled={unit => !!unit.communication_id}
                  onChange={(event, newValue) => {
                    // Pass the new value's id or an empty string to handle the form state
                    onChange(newValue ? newValue.id : '')
                  }}
                  value={communicationData.property_id || null} // Set the selected value
...
export default EditCommunicationDrawer
