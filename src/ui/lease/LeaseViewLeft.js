...
import LeasesSuspendDialog from 'src/ui/leases/LeasesSuspendDialog'
import LeasesSubscriptionDialog from 'src/ui/leases/LeasesSubscriptionDialog'
...
const LeaseViewLeft = ({ leasesData }) => {
  ...
  useEffect(() => {
    if (!leasesData) {
      ...
      //       alert(response.message || 'Failed to fetch leases')

      //       return
      //     }

      //     // setLeasesData(response)
      //   },
      //   error => {}
      // )
    }
      ...
      <Grid container spacing={6}>
        <Grid item xs={12}>
          ...
          <Card>
            <CardContent sx={{ pt: 13.5, display: 'flex', alignItems: 'center', flexDirection: 'column' }}>
              {leasesData?.id ? (
                ...
                <CustomAvatar
                  skin='light'
                  variant='rounded'
                  color={leasesData?.avatarColor}
                  sx={{ width: 100, height: 100, mb: 4, fontSize: '3rem' }}
                >
                  {/* {getInitials(leasesData?.name)} */}
                </CustomAvatar>
              )}
              ...
              <CustomChip
                ...
                skin='light'
                size='small'
                label={leasesData?.status}
                color={statusColors[leasesData?.status]}
            ...
            <CardContent sx={{ pb: 4 }}>
              ...
              <Box sx={{ pt: 4 }}>
                <Box sx={{ display: 'flex', mb: 3 }}>
                  <Typography sx={{ mr: 2, fontWeight: 500 }}>Leases ID:</Typography>
                  <Typography sx={{ color: 'text.secondary' }}>{leasesData?.uuid} </Typography>
                </Box>
                <Box sx={{ display: 'flex', mb: 3 }}>
                  <Typography sx={{ mr: 2, fontWeight: 500 }}>Email:</Typography>
                  <Typography sx={{ color: 'text.secondary' }}>{leasesData?.email}</Typography>
                </Box>
                <Box sx={{ display: 'flex', mb: 3 }}>
                ...
                <Box sx={{ display: 'flex', mb: 3 }}>
                  ...
                  <Typography sx={{ color: 'text.secondary' }}>{leasesData?.tel_number}</Typography>
                </Box>
                <Box sx={{ display: 'flex', mb: 3 }}>
                ...
                <Box sx={{ display: 'flex' }}>
                  ...
                  <Typography sx={{ color: 'text.secondary' }}>{leasesData?.country}</Typography>
                </Box>
              </Box>
            </CardContent>
                  ...
                  <Grid container spacing={6}>
                    ...
                    <Grid item xs={12} sm={6}>
                      <FormControl fullWidth>
                        ...
                        <Select
                          label='Status'
                          defaultValue={leasesData?.status}
                          id='user-view-status'
                    ...
                    <Grid item xs={12} sm={6}>
                      <FormControl fullWidth>
                        ...
                        <Select
                          label='Country'
                          defaultValue={leasesData?.country}
            ...
            <LeasesSuspendDialog open={suspendDialogOpen} setOpen={setSuspendDialogOpen} />
            <LeasesSubscriptionDialog open={subscriptionDialogOpen} setOpen={setSubscriptionDialogOpen} />
          </Card>
        </Grid>
